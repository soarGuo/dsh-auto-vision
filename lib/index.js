...
async function declareImageInputs(settings, namespaces = IMAGE_DECLARATION_NAMESPACES) {
	for (const ns of namespaces) {
		const patch = planImageDeclarations(settings.get(ns), ns);
...
const AUTO_VISION_NAMESPACE = "auto-vision";
...
function apply(ctx, config) {
	...
	const nativeModelNames = () => new Set(nativeVision().map((entry) => entry.model));
	/**
	...
	* 视觉模型,就自动视为原生视觉/识图候选。
	*/
	const matchesNativeVision = (provider, model) => {
	...
	const modelCatalogCache = /* @__PURE__ */ new Map();
	/**
	* 列出某 provider 的模型 id(缓存)。失败或不可用时返回空。
	*/
	const listProviderModelIds = async (provider) => {
		const cached = modelCatalogCache.get(provider);
		if (cached !== void 0) return cached.map((entry) => entry.id);
		try {
			const ids = (await ctx.llm.listModels(provider)).map((entry) => entry.id);
	...
	* 3. 否则回退配置的默认识图路由。
	*/
	const resolveVisionRoute = async (provider) => {
		const reasoningEffort = (current() ?? {}).visionReasoningEffort ?? "high";
		if (provider !== void 0) {
			const exact = nativeVision().find((entry) => entry.provider === provider);
			...
			const byName = (await listProviderModelIds(provider)).find((id) => names.has(id));
	...
	ctx.on("llm/stream", async function* (options, next) {
		...
		if (matchesNativeVision(provider, model)) return yield* next();
