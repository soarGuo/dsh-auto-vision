...
*/
function imagePlaceholderText(index, total) {
	return total > 1 ? `[截图 ${index}/${total} 已由 auto-vision 识别,内容见紧随其后的上下文条目]` : "[截图已由 auto-vision 识别,内容见紧随其后的上下文条目]";
}
/**
	...
	const walk = (blocks) => blocks.flatMap((block) => {
		switch (block.type) {
			case "image":
				...
				return [{
					type: "text",
					text: imagePlaceholderText(seen, total)
				}];
