...
*/
function stripImages(blocks, description) {
	const hasImages = containsImage(blocks);
	const stripped = blocks.flatMap((block) => {
		switch (block.type) {
			case "image": return [];
			case "tool-result": return containsImage(block.content) ? [{
				...block,
				content: stripImages(block.content, "")
			}] : [block];
			default: return [block];
		}
	});
	if (!hasImages) return [...stripped];
	return description.length === 0 ? stripped : [...stripped, {
		type: "text",
		text: description
	}];
}
...
//#endregion
export { AUTO_VISION_NAMESPACE, Config, DEFAULT_NATIVE_VISION, DEFAULT_VISION_MODEL, DEFAULT_VISION_PROVIDER, apply, inject, name };
